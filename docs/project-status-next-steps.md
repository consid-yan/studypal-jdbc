...
项目已经完成了数据库核心结构设计，并且 Java Web 层已经做了一轮适配，让课程、主任务、学生子任务、学习记录等核心页面开始使用新的数据库表结构。
...
| --- | --- | --- | --- | --- |
| 数据库表结构 | 已建立 `user`、`course`、`enrollment`、`main_task`、`sub_task_template`、`student_sub_task`、`study_session` 等核心表。 | 需求文档中提到 `student`、`lecturer` 独立表，但当前实现合并到了 `user.role`。需要统一文档口径。 | 确认最终数据模型：继续使用单一 `user` 表，还是拆出 `student` / `lecturer` profile 表。 | 高 |
| SQL 视图 | 已有优先级、主任务进度、学习效率、每日学习统计、课程负载等视图。 | 页面没有充分使用这些视图。 | 把进度页、Dashboard、教师视图接入 `main_task_progress_view`、`study_efficiency_view`。 | 中 |
| SQL 存储过程 | 已有批量创建模板、复制学生子任务、生成拖延报告等过程。 | Java Service 层还没有系统调用这些存储过程。 | 在创建 MainTask、学生加入课程时调用复制逻辑，或用 Java 事务实现等价逻辑。 | 高 |
| SQL 触发器 | 保留了自动计算 `study_session.duration_hours` 的触发器。 | 为了页面 CRUD，当前移除了禁止 `main_task` 更新/删除的触发器定义；这和需求文档“发布后不可改不可删”冲突。 | 必须二选一：恢复 MainTask 不可编辑/删除，或修改需求文档承认 MainTask 可编辑/删除。 | 高 |
| README 文档 | 已更新当前数据库结构、Java 迁移说明、当前表字段说明。 | 仍有部分说明需要和最新实现再核对，比如 MainTask 是否不可变。 | 在决定 MainTask 规则后同步 README 和 requirements。 | 中 |
| 需求文档 | 已有较完整的角色、任务、AI、进度、学习记录需求。 | 和当前实现存在冲突：MainTask 不可编辑/删除、AI 生成、权限等还没实现。 | 按实际项目目标重审需求，标记“本版本必须做”和“可选扩展”。 | 高 |
...
| SubTask / StudentSubTask 功能 | 页面显示的是学生个人子任务，底层使用 `student_sub_task` 关联 `sub_task_template`。 | 创建子任务时目前偏向手动创建，不是 AI 自动生成；也没有严格区分“模板由教师管理，学生只改个人副本”。 | 实现 MainTask 创建后自动生成模板，并给已选课学生复制个人子任务。 | 高 |
...
| Schedule 功能 | 导航中已隐藏 `/schedule`。 | 新 schema 没有 `schedule_slot`，`ScheduleServlet`、`ScheduleService`、`schedule.jsp` 仍是遗留代码。 | 要么删除 schedule 功能，要么重做成基于 `student_sub_task.planned_start_time/planned_end_time` 的计划视图。 | 低 |
...
当前代码已经完成了核心数据库结构和部分 CRUD 迁移，但还没有达到需求文档中完整的角色权限、AI 生成、自动复制、进度看板和 MainTask 不可变规则。
...
| 后端 DAO/Service 同学 | 实现 MainTask 创建事务：创建任务、生成模板、复制学生子任务。 |
