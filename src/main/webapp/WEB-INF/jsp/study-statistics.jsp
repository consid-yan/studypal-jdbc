...
<title>StudyPal Study Sessions</title>
...
.session-card { transition: transform 0.2s, box-shadow 0.2s; }
.session-card:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(46, 36, 27, 0.1); }
@media (max-width: 960px) {
  ...
  .session-primary { grid-column: span 1 / span 1; }
}
</style>
</head>
...
<div class="student-shell flex min-h-screen">
  <aside class="student-sidebar fixed top-0 left-0 w-[240px] h-screen bg-card border-r border-border flex flex-col z-40">
    ...
    <nav class="student-nav flex-1 p-4 space-y-1">
      ...
      <a href="${pageContext.request.contextPath}/study-sessions?role=STUDENT" class="sidebar-link active">
  ...
  <div class="student-main ml-[240px] flex-1 flex flex-col">
    <header class="sticky top-0 z-30 bg-cream/90 backdrop-blur border-b border-border px-8 py-5">
      <div class="flex items-center justify-between gap-4 flex-wrap">
        <div>
          <h1 class="text-xl font-bold text-textDark">Study Sessions</h1>
          <p class="text-sm text-textMuted mt-0.5">Record planned or actual study time and connect it to your StudentSubTask progress.</p>
        </div>
    ...
    <main class="flex-1 p-8">
      <div class="max-w-[1160px] mx-auto">
        <div class="session-stats grid grid-cols-4 gap-5 mb-8 fade-in">
          <div class="warm-card p-5 session-card">
            <p class="text-xs text-textMuted mb-1">This Week</p>
            <p class="text-2xl font-bold text-primary">18.5h</p>
            <p class="text-xs text-textMuted mt-1">Recorded study time</p>
          </div>
          <div class="warm-card p-5 session-card">
            <p class="text-xs text-textMuted mb-1">Sessions</p>
            <p class="text-2xl font-bold text-accent">7</p>
            <p class="text-xs text-textMuted mt-1">Actual and planned logs</p>
          </div>
          <div class="warm-card p-5 session-card">
            <p class="text-xs text-textMuted mb-1">Linked Tasks</p>
            <p class="text-2xl font-bold text-primary">5</p>
            <p class="text-xs text-textMuted mt-1">Connected to StudentSubTask</p>
          </div>
          <div class="warm-card p-5 session-card">
            <p class="text-xs text-textMuted mb-1">Focus Rate</p>
            <p class="text-2xl font-bold text-accent">82%</p>
            <p class="text-xs text-textMuted mt-1">Based on completed sessions</p>
          </div>
        </div>

        <div class="session-grid grid grid-cols-3 gap-6">
          <section class="session-primary col-span-2 space-y-6">
            <div class="warm-card p-6 fade-in-d1">
              <h2 class="text-base font-bold text-textDark mb-4">Record StudySession</h2>
              <form method="post" action="${pageContext.request.contextPath}/study-sessions" data-validate="time-range" class="space-y-4">
                <input type="hidden" name="action" value="create">
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    <label class="block text-xs font-medium text-textMuted mb-1">Start Time</label>
                    <input type="datetime-local" name="startTime" data-start-time required class="w-full px-3 py-2 text-sm bg-cream border border-border rounded-lg focus:outline-none focus:border-primary">
                  </div>
                  <div>
                    <label class="block text-xs font-medium text-textMuted mb-1">End Time</label>
                    <input type="datetime-local" name="endTime" data-end-time required class="w-full px-3 py-2 text-sm bg-cream border border-border rounded-lg focus:outline-none focus:border-primary">
                  </div>
                </div>
                <div class="grid grid-cols-2 gap-4">
                  <div>
                    <label class="block text-xs font-medium text-textMuted mb-1">Type</label>
                    <select name="sessionType" class="w-full px-3 py-2 text-sm bg-cream border border-border rounded-lg focus:outline-none focus:border-primary">
                      <option value="ACTUAL">ACTUAL</option>
                      <option value="PLANNED">PLANNED</option>
                    </select>
                  </div>
                  <div>
                    <label class="block text-xs font-medium text-textMuted mb-1">Linked StudentSubTask</label>
                    <select name="studentSubTaskId" class="w-full px-3 py-2 text-sm bg-cream border border-border rounded-lg focus:outline-none focus:border-primary">
                      <option value="">Database ER Diagram Review</option>
                      <option value="">Packet Tracer Assignment</option>
                      <option value="">Research Presentation</option>
                    </select>
                  </div>
                </div>
                <div>
                  <label class="block text-xs font-medium text-textMuted mb-1">Notes</label>
                  <textarea name="notes" rows="3" class="w-full px-3 py-2 text-sm bg-cream border border-border rounded-lg focus:outline-none focus:border-primary resize-none">Finished the relationship review and updated progress notes.</textarea>
                </div>
                <div class="flex gap-3 flex-wrap">
                  <button class="action-btn action-btn-primary" type="submit">Save StudySession</button>
                  <a href="${pageContext.request.contextPath}/sub-tasks?role=STUDENT" class="action-btn action-btn-secondary">Back to My Tasks</a>
                </div>
              </form>
            </div>
            ...
            <div class="warm-card p-6 fade-in-d2">
              <div class="flex items-center justify-between gap-3 mb-4">
                <h2 class="text-base font-bold text-textDark">Recent Records</h2>
                <span class="text-xs text-textMuted">Latest StudySession logs</span>
              </div>
              <div class="space-y-3">
                <div class="p-4 rounded-xl bg-cream border border-border/60">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <p class="text-xs text-textMuted mt-0.5">May 27, 19:00 - 21:00 · 2.0h</p>
                    </div>
                    <span class="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium">ACTUAL</span>
                  </div>
                  <p class="text-xs text-textMuted mt-3">Completed database structure confirmation and reviewed the StudentSubTask chain.</p>
                </div>
                <div class="p-4 rounded-xl bg-cream border border-border/60">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      <p class="text-sm font-semibold text-textDark">Frontend Static Prototype</p>
                      <p class="text-xs text-textMuted mt-0.5">May 28, 14:00 - 15:30 · 1.5h</p>
                    </div>
                    <span class="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-medium">ACTUAL</span>
                  </div>
                  <p class="text-xs text-textMuted mt-3">Finished the student page frame and checked sidebar consistency.</p>
                </div>
                <div class="p-4 rounded-xl bg-cream border border-border/60">
                  <div class="flex items-start justify-between gap-4">
                    <div>
                      <p class="text-sm font-semibold text-textDark">Packet Tracer Assignment</p>
                      <p class="text-xs text-textMuted mt-0.5">May 29, 09:00 - 11:00 · planned</p>
                    </div>
                    <span class="text-xs bg-accent/15 text-accent px-2 py-0.5 rounded-full font-medium">PLANNED</span>
                  </div>
                  <p class="text-xs text-textMuted mt-3">Prepare VLAN screenshots and command output evidence.</p>
                </div>
              </div>
            </div>
          </section>
          ...
          <aside class="space-y-6">
            <section class="warm-card p-5 fade-in-d1">
              <h3 class="text-sm font-bold text-textDark mb-4">Weekly Rhythm</h3>
              <div class="space-y-4">
                <div>
                  <div class="flex items-center justify-between mb-1">
                    <span class="text-xs text-textMuted">Database Systems</span>
                    <span class="text-xs font-semibold text-primary">7.5h</span>
                  </div>
                  <div class="h-2 bg-border/60 rounded-full overflow-hidden"><div class="h-full bg-primary rounded-full progress-bar" style="--target-width: 82%;"></div></div>
                </div>
                <div>
                  <div class="flex items-center justify-between mb-1">
                    <span class="text-xs text-textMuted">Computer Networks</span>
                    <span class="text-xs font-semibold text-accent">5.0h</span>
                  </div>
                  <div class="h-2 bg-border/60 rounded-full overflow-hidden"><div class="h-full bg-accent rounded-full progress-bar" style="--target-width: 58%;"></div></div>
                </div>
                <div>
                  <div class="flex items-center justify-between mb-1">
                    <span class="text-xs text-textMuted">Academic English</span>
                    <span class="text-xs font-semibold text-primary">6.0h</span>
                  </div>
                  <div class="h-2 bg-border/60 rounded-full overflow-hidden"><div class="h-full bg-primary rounded-full progress-bar" style="--target-width: 70%;"></div></div>
                </div>
              </div>
            </section>

            <section class="warm-card p-5 fade-in-d2">
              <h3 class="text-sm font-bold text-textDark mb-4">Next Planned Session</h3>
              <div class="rounded-xl bg-cream border border-border/60 p-4">
                <p class="text-xs text-accent font-semibold">Tomorrow · 09:00</p>
                <p class="text-sm text-textDark font-semibold mt-1">Computer Networks</p>
                <p class="text-xs text-textMuted mt-1">Upload VLAN and trunk verification screenshots.</p>
              </div>
              <a href="${pageContext.request.contextPath}/sub-tasks?role=STUDENT" class="action-btn action-btn-secondary w-full mt-4">View Linked Task</a>
            </section>
          </aside>
        </div>
