...
<%
    ...
    List<Course> myCourses = null; int tcs = 0, tss = 0; double avgP = 0;
    try {
        ...
        for (Course c : myCourses) { avgP += ts.getTaskAvgCompletion(c.getCourseId()); }
        if (myCourses != null && !myCourses.isEmpty()) avgP /= myCourses.size();
...
<div class="workspace-shell">
  <aside class="workspace-sidebar">
    ...
    <nav class="workspace-nav">
      ...
      <a class="sidebar-link active" href="${pageContext.request.contextPath}/lecturer-home.jsp?role=LECTURER"><svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2"><path d="M2 3h12v10H2z"/><path d="M5 1v4"/></svg>My Courses</a>
  ...
  <div class="workspace-main">
    ...
    <main class="workspace-content"><div class="content-inner">
      <section class="stats-grid fade-in">
        ...
        <div class="warm-card stat-card"><p class="stat-label">Tasks</p><p class="stat-value"><%= ts.getPublishedTaskCount(cu.getUserId()) %></p><p class="stat-hint">Active MainTasks</p></div>
          ...
          <div class="course-list-stack" style="margin-top:16px">
            <% if (myCourses != null && !myCourses.isEmpty()) {
                 for (Course c : myCourses) {
                   double cp = 0; try { cp = ts.getTaskAvgCompletion(c.getCourseId()); } catch (Exception ignored) {}
            %>
